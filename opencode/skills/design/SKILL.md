---
name: design
description: Use when building any UI, component, page, or visual interface. Produces distinctive design with a committed aesthetic, not generic defaults. Not for backend logic or data pipelines.
version: 3.0.0
allowed-tools:
  - read
  - apply_patch
...
Before writing a single line of code, ask the user for answers to these four questions:
...
- No sample content without a comment explaining what replaces it
...
Before submitting, reload `references/design-reference.md` and check the common traps list. Then open the result in a browser and confirm it looks right before handoff. Code that looks correct in your head but wrong in the browser is not done.
...
- Clear instructions for replacing sample content with real content
