---
name: think
description: Use when planning a new feature, design, or architecture decision before writing code. Turns rough ideas into approved plans with validated structure. Not for routine bug fixes or small edits.
version: 3.1.0
allowed-tools:
  - read
  - grep
...
Start by running `git log --oneline -10` and reading any repo instructions file if present, such as a root-level agent guidance file. Then read the files the user mentioned or that are obviously related to the idea (entry points, main modules). Ask if it is unclear which files are relevant. Then work through the idea one question at a time: purpose first, constraints second, success criteria third.
...
| **hold** | Existing direction is fixed, tight constraints | Scope is locked. Make it correct. |
...
**Scope.** Search for existing implementations of each sub-problem. Flag anything deferrable. More than 8 files or 2 new services? Acknowledge it explicitly.
...
**Test coverage.** List every meaningful path: happy path, error branches, edge cases. List gaps with file, assertion, test type. Any corrective change without a reproducing test is not done.
...
**No gaps in approved plans.** Before the user approves, every step must be concrete. Forbidden patterns: "implement later", "similar to step N", "details to be determined", "as needed", or any unresolved marker. A plan with gaps is not a plan. It is a promise to plan later.
...
3. **Unknowns are resolved or explicitly deferred?** No unresolved markers hidden in the plan.
